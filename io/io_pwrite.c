...
{
	IO_LOG(TRACE, "io pwrite with iodes=%p, nbytes=%zu", iodes, nbytes);
	...
	if (nbytes == 0) {
		IO_LOG(TRACE, "read/write size is 0, nothing to do");
		return (0);
	}
	...
	IO_LOG(TRACE, "reading %zu bytes from iodes_2", nbytes);
	if ((rbytes = io_read(iodes_2, buf, nbytes)) < 0) {
		IO_LOG(ERROR, "read from iodes_2 failed");
		goto label_exit;
	}
	IO_LOG(TRACE, "read %zu bytes from iodes_2", rbytes);
	
	IO_LOG(TRACE, "writing %zu bytes to iodes_1", rbytes);
	if ((wbytes = io_write(iodes_1, buf, rbytes)) < 0) {
		IO_LOG(ERROR, "write to iodes_1 failed");
		goto label_exit;
	}
