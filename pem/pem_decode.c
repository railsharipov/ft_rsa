...
int pem_decode(t_pem *pem, t_ostring *enc, t_ostring *data, const char *label, const char *pass)
{
	...
	char		buf[1024], vecthex[16], *cipher_name, *proc_type;
	...
	pos = 0;
	seek = textutil_seekf(enc->content + pos, enc->size - pos, "-----BEGIN %s-----", label);
	if (seek <= 0) {
	...
	matches = textutil_sscanf(enc->content + pos, enc->size - pos, "Proc-Type: %s\n", &proc_type);
	...
	if (pem->proc == PEM_PROC_TYPE_ENCRYPTED) {
		matches = textutil_sscanf(enc->content + pos, enc->size - pos, "DEK-Info: %s,%s\n", &cipher_name, &vecthex);
		if (matches != 2) {
		...
		if (pem->cipher != PEM_CIPHER_DES_CBC) {
			...
			pos += textutil_seekf(enc->content + pos, enc->size - pos, "DEK-Info: DES-CBC,%s\n", vecthex);
		}
		ft_hex_to_bytes(pem->iv, vecthex, 16);
	}
	pos += textutil_seekf(enc->content + pos, enc->size - pos, "\n");
	
	idx = textutil_findf(enc->content + pos, enc->size - pos, "%s%s%s", "-----END ", label, "-----");
	if (idx < 0) {
		PEM_LOG(ERROR, "bad pem encoding: encapsulation end");
		goto label_exit;
	}

	ft_ostr_append(&b64enc, enc->content + pos, idx - pos);
	...
	if (pem->proc == PEM_PROC_TYPE_ENCRYPTED) {
		if (SSL_OK != rand_openssl_kdf(pem->key, pem->iv, pem->salt, pass)) {
			PEM_LOG(ERROR, "bad pem encoding: bad key derivation");
			goto label_exit;
		}
	
		des = des_init(pem->key, NULL, pem->salt);
