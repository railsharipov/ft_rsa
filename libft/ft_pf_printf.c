...
void	pf_flags(char *len, char *flag, char *type)
{
...
void	pf_format(t_pf *data, va_list *arg)
{
	pf_flags(&(data->len), &(data->flag), &(data->type));
...
void	pf_parse(t_pf *data, const char *s, va_list *arg)
{
	int	i;
	int	start;
