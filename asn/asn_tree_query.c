...
int asn1_tree_query(const char *s, t_node *asn1_tree, t_node **ret_asn1_node)
{
	...
	if (NULL == asn1_tree || NULL == ret_asn1_node) {
		SSL_LOG(ERROR, INVALID_INPUT_ERROR);
		return (SSL_ERR);
	}

	SSL_LOG(TRACE, "running query `%s` on asn tree: %s", s, asn1_tree_dump(asn1_tree));

	*ret_asn1_node = NULL;

	if (SSL_OK != json_query_with_f_selector(s, (t_node *)asn1_tree, (t_node **)ret_asn1_node, __f_asn1_node_selector)) {
		SSL_LOG(ERROR, __ASNQ_BAD_QUERY_ERROR);
		return (SSL_ERR);
	}

	SSL_LOG(TRACE, "query result: %s", asn1_tree_dump(*ret_asn1_node));
