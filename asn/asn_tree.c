...
t_node	*asn_tree_create(t_node *json_schema)
{
	...
	SSL_LOG(TRACE, "json schema: %s", asn_tree_dump(json_schema));
	...
	SSL_LOG(TRACE, "setting encode tag for asn item with tagnum: %#x", asn_item->tagnum);
	...
	SSL_LOG(TRACE, "using asn_item encode tag: %s", SSL_FLAG(ASN_ENCODE_CONSTRUCT, asn_item->tag) ? "construct" : "primitive");
	...
	if (asn_item->encapsulates == SSL_TRUE) {
		if (asn_item->tagnum != ASN_TAGNUM_OCTET_STRING && asn_item->tagnum != ASN_TAGNUM_BIT_STRING) {
			SSL_LOG(ERROR, "bad schema: expected data to be encapsulated in octet or bit string but got %s", asn_item_get_type_name(asn_item));
			goto label_error;
		}
	}

	asn_node = asn_node_create(asn_item);
	SSL_LOG(TRACE, "successfully created asn node: %s", asn_tree_dump(asn_node));
