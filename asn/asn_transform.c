#include <common.h>
#include <logger.h>
#include <asn1.h>
#include <libft.h>

int	asn_transform(t_node *src_tree, t_node *dest_tree)
{
	t_htbl	*dest_items;
	t_node	*src_iter;
	t_node	*src_ref;
	t_node	*src_node;
	t_iasn	*src_item;
	t_iasn	*dest_item;

	if (NULL == src_tree || NULL == dest_tree) {
		SSL_LOG(ERROR, INVALID_INPUT_ERROR);
		return (SSL_ERR);
	}
	src_iter = ft_ntree_iter(src_tree);
	dest_items = ft_ntree_to_htable(dest_tree);
	src_ref = src_iter;

	while (NULL != src_iter) {
		src_node = src_iter->content;
		dest_item = ft_htbl_get(dest_items, src_node->key);

		if (NULL != dest_item) {
			src_item = src_node->content;

			if (NULL != src_item) {
				ft_memcpy(dest_item->content, src_item->content, src_item->size);
				dest_item->size = src_item->size;
				ft_strcpy(dest_item->description, src_item->description);
				dest_item->tag = src_item->tag;
				dest_item->tagnum = src_item->tagnum;
			} else {
				SSL_LOG(ERROR, "invalid asn tree");
				return (SSL_ERR);
			}
		}
		src_iter = src_iter->next;
	}
	ft_lst_del(src_ref);

	return (SSL_OK);
}
