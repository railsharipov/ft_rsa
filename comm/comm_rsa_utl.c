...
static int	__get_input(t_io *io, char **input, size_t *insize)
{
	...
	*insize = 0;
	...
	while ((rbytes = io->func(io, buf, RD_BUF)) > 0)
	{
...
static int	__decode_key(t_pem *key, t_node **asn_key)
{
	...
	if (SSL_OK != pem_decode(key, __in_type, &der_key))
	{
		ret = SSL_ERROR("invalid rsa private key");
	}
...
static int	__run_task(void)
{
	...
	{
		return (SSL_ERROR("rsa keyfile not specified"));
	}
	if (SSL_OK != __get_input(&__inkey, &(key.content), &(key.size)))
	{
		ret = SSL_ERROR("rsautl input error");
	}
	...
	{
		ret = SSL_ERROR("rsautl output error");
	}
...
int	comm_rsa_utl(char **opt, const char *name_comm)
{
	int	ret;

	if (NULL == opt)
	{
		return (SSL_ERROR("rsautl command error"));
	}
